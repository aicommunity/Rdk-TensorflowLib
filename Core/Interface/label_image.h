...
static Status ReadTensorFromImageFile(const string& file_name, const int input_height,
...
static Status GetTopLabels(const std::vector<Tensor>& outputs, int how_many_labels,
...
static Status PrintTopLabels(const std::vector<Tensor>& outputs,
...
// one that's expected.
static Status CheckTopLabel(const std::vector<Tensor>& outputs, int expected,bool* is_expected)
{
  *is_expected = false;
  Tensor indices;
  Tensor scores;
